...
class TelegramUser(AbstractUser):
    ...
    telegram_first_name = models.CharField(
        max_length=100, blank=True, verbose_name="Telegram First Name"
    )
    telegram_last_name = models.CharField(
        max_length=100, blank=True, verbose_name="Telegram Last Name"
    )

    # Дополнительные поля
    avatar_url = models.URLField(blank=True, verbose_name="Avatar URL")
    language_code = models.CharField(
        max_length=10, default="ru", verbose_name="Language Code"
    )
    ...
    def __str__(self):
        return (
            f"{self.telegram_first_name} (@{self.telegram_username})"
