...
def telegram_auth_callback(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "Неверный формат JSON данных"}, status=400
            )
...
def auth_error(request):
    ...
    return render(request, "telegram_auth/error.html", {"error_message": error_message})
...
class TelegramAuthAPI(View):
    ...
    def get(self, request):
        ...
        if not session_token:
            return JsonResponse(
                {"success": False, "message": "Токен сессии не предоставлен"},
                status=401,
            )
        ...
        if not user:
            return JsonResponse(
                {"success": False, "message": "Недействительная сессия"}, status=401
            )
...
def telegram_auth_status(request):
    ...
    if not session_token:
        return JsonResponse(
            {"authenticated": False, "message": "Токен не предоставлен"}
        )
    ...
    if user:
        ...
        return JsonResponse(
            {"authenticated": False, "message": "Сессия недействительна или истекла"}
        )
