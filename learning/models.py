...
class Subject(models.Model):
    ...
    code = models.CharField(
        max_length=50, blank=True, default="", verbose_name="Код предмета"
    )
    ...
    icon = models.CharField(
        max_length=100, blank=True, default="", verbose_name="Иконка"
    )
    is_archived = models.BooleanField(
        default=False, verbose_name="Архивирован"
    )  # type: ignore
    is_primary = models.BooleanField(
        default=False, verbose_name="Основной предмет"  # type: ignore
    )  # type: ignore
...
class Task(models.Model):
    ...
    source = models.CharField(
        max_length=200, blank=True, null=True, verbose_name="Источник"
    )
...
class UserRating(models.Model):
    ...
    incorrect_answers = models.IntegerField(
        default=0, verbose_name="Неправильных ответов"
    )  # type: ignore
    total_attempts = models.IntegerField(
        default=0, verbose_name="Всего попыток"
    )  # type: ignore
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Дата обновления")
...
class Achievement(models.Model):
    ...
    points_required = models.IntegerField(
        default=0, verbose_name="Требуемые очки"
    )  # type: ignore
    users = models.ManyToManyField(
        settings.AUTH_USER_MODEL, through="UserAchievement", verbose_name="Пользователи"
    )
