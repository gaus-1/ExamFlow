...
def math_subject_detail(request, subject_id):
    ...
    try:
        ...
        return render(
            request,
            "learning/subject_not_found.html",
        ...
        return render(
            request,
            "learning/subject_error.html",
            {"message": "Ошибка при загрузке данных по математике.", "details": str(e)},
            status=500,
        )
...
def focused_search(request):
    ...
    ).filter(Q(title__icontains=query) | Q(description__icontains=query))[:10]
...
def get_subject_statistics(request):
    ...
    math_subjects = Subject.objects.filter(  # type: ignore
        name__icontains="математика", is_archived=False, is_primary=True
    )

    russian_subjects = Subject.objects.filter(  # type: ignore
        name__icontains="русский", is_archived=False, is_primary=True
    )
