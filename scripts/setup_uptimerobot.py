...
class UptimeRobotSetup:
    ...
    def create_monitor(
        self, name: str, url: str, monitor_type: int = 1
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if result.get("stat") == "ok":
                ...
                print(
                    f"❌ Ошибка создания монитора '{name}': {result.get('error', {}).get('message', 'Unknown error')}"
                )
                return None
    ...
    def get_monitors(self) -> dict[str, Any] | None:
        """Получает список всех мониторов"""
        data = {"api_key": self.api_key, "format": "json"}
        ...
        try:
            ...
            if result.get("stat") == "ok":
                ...
                print(
                    f"❌ Ошибка получения мониторов: {result.get('error', {}).get('message', 'Unknown error')}"
                )
                return None
    ...
    def delete_monitor(self, monitor_id: str) -> bool:
        """Удаляет монитор"""
        data = {"api_key": self.api_key, "format": "json", "id": monitor_id}
        ...
        try:
            ...
            if result.get("stat") == "ok":
                ...
                print(
                    f"❌ Ошибка удаления монитора {monitor_id}: {result.get('error', {}).get('message', 'Unknown error')}"
                )
                return False
...
def main():
    ...
    api_key = input(
        "Введите API ключ UptimeRobot (или нажмите Enter для пропуска): "
    ).strip()
    ...
    if not api_key:
        ...
        print(
            "2. Перейдите в Settings: https://uptimerobot.com/dashboard.php#mySettings"
        )
