...
class UptimeRobotSetup:
    ...
    def create_monitor(self, name: str, url: str, monitor_type: int = 1) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.get('stat') == 'ok':
                print("✅ Монитор '{name}' создан успешно")
                return result.get('monitor')
            else:
                print("❌ Ошибка создания монитора '{name}': {result.get('error', {}).get('message', 'Unknown error')}")
                return None

        except Exception as e:
            print("❌ Ошибка при создании монитора '{name}': {e}")
            return None
    ...
    def get_monitors(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.get('stat') == 'ok':
                ...
                print("❌ Ошибка получения мониторов: {result.get('error', {}).get('message', 'Unknown error')}")
                return None

        except Exception as e:
            print("❌ Ошибка при получении мониторов: {e}")
            return None
    ...
    def delete_monitor(self, monitor_id: str) -> bool:
        ...
        try:
            ...
            if result.get('stat') == 'ok':
                print("✅ Монитор {monitor_id} удален")
                return True
            else:
                print("❌ Ошибка удаления монитора {monitor_id}: {result.get('error', {}).get('message', 'Unknown error')}")
                return False

        except Exception as e:
            print("❌ Ошибка при удалении монитора {monitor_id}: {e}")
            return False
...
def setup_examflow_monitoring(api_key: str):
    ...
    for monitor in created_monitors:
        print("   - {monitor.get('friendly_name')} (ID: {monitor.get('id')})")
