...
def optimize_all_images(static_dir: str = 'static/images'):
    ...
    if not static_path.exists():
        logger.warning(f"Директория {static_dir} не найдена")
        return
        ...
        with connection.cursor() as cursor:
            ...
            for index_sql in indexes:
                try:
                    ...
                    self.stdout.write(f'✅ Индекс создан: {index_sql.split()[5]}')
                except Exception as e:
                    self.stdout.write(f'⚠️ Ошибка создания индекса: {e}')
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"Performance: {name} - {execution_time:.3f}s, {query_count} queries")
...
def log_slow_queries():
    ...
    if slow_queries:
        ...
        for query in slow_queries:
            logger.warning(f"  {query['time']}s: {query['sql']}")
'''
