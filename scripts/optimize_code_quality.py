...
def add_function_type_hints(content: str) -> str:
    ...
    def add_hints(match):
        ...
        if func_name in ["post", "get"]:
            return "def {func_name}(self, request: HttpRequest) -> JsonResponse:"
        elif func_name in ["process_query", "generate_prompt"]:
            return "def {func_name}(self, query: str, user_id: Optional[int] = None) -> Dict[str, Any]:"
        elif func_name in ["create_embedding", "search"]:
            return "def {func_name}(self, text: str, limit: int = 5) -> List[Dict[str, Any]]:"
        else:
...
def add_docstrings_to_file(file_path: str):
    ...
    print("📚 Добавляем docstrings в {file_path}...")
