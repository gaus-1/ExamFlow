...
class RAGService:
    ...
    def analyze_student_progress(
            self, user: User, subject: Optional[Subject] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            weak_topics = [
                name for name,
                stats in topic_stats.items() if stats['accuracy'] < 60]
            strong_topics = [
                name for name,
                stats in topic_stats.items() if stats['accuracy'] >= 80]
    ...
    def generate_personalized_prompt(
            ...
            user: User,
            task: Task,
            task_type: str = 'task_explanation') -> str:
        """
