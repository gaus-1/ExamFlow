...
class VoiceService:
    ...
    def text_to_speech(self, text, task_id=None, voice_type='task'):
        ...
        try:
            ...
            if cached_path and os.path.exists(os.path.join(settings.MEDIA_ROOT, cached_path)):
...
def generate_task_voices():
    ...
    tasks = Task.objects.filter(is_active=True, audio_file__isnull=True)[:50]  # Ограничиваем для экономии
    generated_count = 0
