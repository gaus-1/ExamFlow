    ...
    def get_or_create_profile(telegram_id: int, telegram_username: str = None, user=None): # type: ignore
        ...
        try:
            ...
            if profile:
                ...
                logger.debug(f"Найден существующий профиль для telegram_id={telegram_id}")
                return profile
            ...
            return UnifiedProfileService._create_mock_profile(telegram_id, telegram_username)
    ...
    def update_profile_activity(profile, activity_type: str = "message"):
        ...
        try:
            if hasattr(profile, 'save'):
                ...
                logger.debug(f"Обновлена активность профиля {profile.telegram_id}: {activity_type}")
