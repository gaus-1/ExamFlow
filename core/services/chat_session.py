...
class ChatSessionService:
    ...
    def get_or_create_chat_session(user_profile, session_type: str = "general"):
        ...
        try:
            ...
            session = ChatSession.objects.filter(
                user_profile=user_profile, session_type=session_type, is_active=True
            ).first()

            if session:
                logger.debug(
                    f"Найдена активная сессия {session.id} для пользователя {user_profile.telegram_id}"
                )
                return session
            ...
            logger.info(
                f"Создана новая сессия {session.id} для пользователя {user_profile.telegram_id}"
            )
            return session
    ...
    @staticmethod
    def update_session(
        session_id: str, message_role: str, message_content: str
    ) -> bool:
        try:
            from core.models import ChatSession
    ...
    @staticmethod
    def add_assistant_message_to_session(
        session, message: str, metadata: dict[str, Any] = None
    ):
    ...
    def create_enhanced_prompt(
        session, user_message: str, context_limit: int = 5
    ) -> str:
        """Создает расширенный промпт с контекстом сессии"""
        try:
            # Получаем последние сообщения для контекста
            recent_messages = ChatSessionService.get_session_messages(
