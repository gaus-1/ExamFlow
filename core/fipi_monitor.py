...
class FIPIMonitor:
    ...
    def check_for_updates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Проверка завершена. Найдено обновлений: {updates['total_updates']}")
        ...
        try:
            ...
            if content_hash != last_hash:
                ...
                for subject in self.subjects:
                    ...
                    if subject_updates:
                        updates[subject.lower().replace(' ', '_')].extend(subject_updates)
    ...
    def _check_open_bank(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            if ege_hash != last_ege_hash:
                ...
                for subject in self.subjects:
                    subject_updates = self._analyze_bank_changes(ege_content, subject, 'ЕГЭ')
                    if subject_updates:
                        updates[subject.lower().replace(' ', '_')].extend(subject_updates)
            ...
            if oge_hash != last_oge_hash:
                ...
                for subject in self.subjects:
                    subject_updates = self._analyze_bank_changes(oge_content, subject, 'ОГЭ')
                    if subject_updates:
                        updates[subject.lower().replace(' ', '_')].extend(subject_updates)
    ...
    def _analyze_bank_changes(self, content: str, subject: str, exam_type: str) -> List[Dict]:
    ...
    def _save_updates_to_db(self, updates: Dict[str, Any]):
        ...
        try:
            for subject_updates in [updates['math_updates'], updates['russian_updates']]:
