...
class KeepaliveService:
        ...
        try:
            ...
            for endpoint in endpoints:
                try:
                    ...
                    if response.status_code in [200, 404, 500]:  # Любой ответ лучше чем таймаут
                        success_count += 1
            ...
            if success:
                logger.info(f"✅ Сайт разбужен успешно ({success_count}/{len(endpoints)} эндпоинтов)")
            else:
                logger.warning(f"⚠️ Сайт частично разбужен ({success_count}/{len(endpoints)} эндпоинтов)")
        ...
        while self.is_running:
            try:
                ...
                if overall_status != 'healthy':
                    logger.warning("⚠️ Система нездорова, пытаемся разбудить компоненты...")
    ...
    def _send_alert(self, health_results: Dict[str, Any]):
        ...
        logger.error(f"🚨 КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ: {self.stats['consecutive_failures']} последовательных неудач")
