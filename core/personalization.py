...
from learning.models import Subject, Task, UserProgress, Topic
...
class UserBehaviorAnalyzer:
    ...
    def analyze_behavior(self):
        ...
        try:
            ...
            logger.error(f"Ошибка анализа поведения пользователя {self.user.username}: {e}")
            return {}
    ...
    def _analyze_subject_preferences(self):
        ...
        try:
            ...
            subject_progress = UserProgress.objects.filter( # type: ignore
                user=self.user
            ).select_related('task__subject').values(
                'task__subject__name'
            ).annotate(
    ...
    def _analyze_task_preferences(self):
        ...
        try:
            ...
            difficulty_preferences = UserProgress.objects.filter( # type: ignore
                user=self.user
            ).select_related('task').values(
                'task__difficulty'
            ).annotate(
                total_attempts=Count('id'),
                success_rate=Count('id', filter=Q(is_correct=True)) * 100.0 / Count('id')
            )
            
            # Анализируем предпочтения по темам
            topic_preferences = UserProgress.objects.filter( # type: ignore
                user=self.user
            ).select_related('task__subject').values(
                'task__subject__name'
            ).annotate(
    ...
    def _analyze_time_patterns(self):
        ...
        try:
            ...
            recent_attempts = UserProgress.objects.filter( # type: ignore
                user=self.user,
                last_attempt__gte=timezone.now() - timedelta(days=30)
    ...
    def _get_preferred_difficulty(self):
        ...
        try:
            difficulty_stats = UserProgress.objects.filter( # type: ignore
                user=self.user
            ).select_related('task').values(
                'task__difficulty'
            ).annotate(
    ...
    def _get_preferred_topics(self):
        ...
        try:
            topic_stats = UserProgress.objects.filter( # type: ignore
                user=self.user
            ).select_related('task__subject').values(
                'task__subject__name'
            ).annotate(
    ...
    def _determine_learning_style(self):
        ...
        try:
            ...
            quick_answers = UserProgress.objects.filter( # type: ignore
                user=self.user,
                last_attempt__gte=timezone.now() - timedelta(days=7)
            ).count()
    ...
    def _determine_difficulty_level(self):
        ...
        try:
            recent_progress = UserProgress.objects.filter( # type: ignore
                user=self.user,
                last_attempt__gte=timezone.now() - timedelta(days=14)
            )
            
            if not recent_progress.exists():
                return 'medium'
            
            success_rate = recent_progress.filter(is_correct=True).count() / recent_progress.count()
...
class PersonalizedRecommendations:
    ...
    def get_recommendations(self, limit=10):
        ...
        try:
            ...
            subject_recommendations = self._get_subject_recommendations(behavior_profile)
