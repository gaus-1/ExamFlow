...
class KeepAliveService:
    ...
    def start_keepalive(self):
        ...
        try:
            ...
            logger.warning(
                "Не удалось подключить weekly_reminders, пропускаю планирование")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                site_response = requests.get(
                    f"{self.site_url}/api/statistics/", timeout=30)
                ...
                if site_response.status_code == 200:
                    ...
                    logger.warning(
                        f"⚠️ Сайт недоступен: код {site_response.status_code}")
                    if self.chat_id:
                        self.send_admin_notification(
                            f"🔴 Сайт недоступен: {site_response.status_code}")
                    return False
            else:
