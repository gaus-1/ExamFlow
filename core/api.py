...
class EmergencyAIView(View):
    ...
    def post(self, request):
        try:
            ...
            if not query:
                return JsonResponse(
                    {"success": False, "error": "Пустой запрос"}, status=400
                )
            ...
            try:
                ...
                answer = (
                    ai_result.get("answer") if isinstance(ai_result, dict) else None
                )
            except Exception:
                answer = None
            ...
            return JsonResponse(
                {
                    "success": True,
                    "answer": "Сервис временно перегружен. Попробуйте позже.",
                },
                status=200,
            )
...
def get_tasks_by_subject(request, subject_id):
    ...
    try:
        ...
        tasks = list(
            Task.objects.filter(subject_id=subject_id).values(  # type: ignore
...
def get_task_by_id(request, task_id):
    ...
    try:
        ...
        task = (
            ...
            .values("id", "title", "description", "subject_id")  # type: ignore
            .first()
        )
        if not task:
...
def get_subject_detail(request, subject_id):
    ...
    try:
        ...
        subject = (
            ...
            .values("id", "name", "exam_type", "code")  # type: ignore
            .first()
        )
        if not subject:
            return JsonResponse({"error": "Not found"}, status=404)
