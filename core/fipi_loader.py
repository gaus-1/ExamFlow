            ...
            for exam_type_name, subjects in self.subjects_data.items():
                ...
                for subject_key, subject_data in subjects.items():
                    try:
                        ...
                        if created:
                            total_subjects += 1
                            logger.info(f"Создан предмет: {subject.name} ({exam_type_name})")
                        
                        # Загружаем материалы для предмета
                        tasks_count = self._load_subject_materials(subject, subject_data)
                        ...
                        logger.error(f"Ошибка при загрузке предмета {subject_data['name']}: {str(e)}")
                        continue
            
            logger.info(f"Загрузка завершена. Предметов: {total_subjects}, заданий: {total_tasks}")
    ...
    def create_sample_tasks(self):
        ...
        for subject_name, tasks in sample_tasks.items():
            try:
                ...
                logger.error(f"Ошибка при создании примеров для {subject_name}: {str(e)}")
