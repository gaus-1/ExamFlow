...
from learning.models import ExamType, Subject, Task, Topic
...
class FipiLoader:
    ...
    def load_subjects(self):
        ...
        with transaction.atomic():
            ege, _ = ExamType.objects.get_or_create(  # type: ignore
                code="EGE", defaults={"name": "Единый государственный экзамен"}
            )
            oge, _ = ExamType.objects.get_or_create(  # type: ignore
                code="OGE", defaults={"name": "Основной государственный экзамен"}
            )
            ...
            for exam_type_name, subjects in self.subjects_data.items():
                _ = exam_types[exam_type_name]

                for _subject_key, subject_data in subjects.items():
                    try:
    ...
    def create_sample_tasks(self):
        ...
        sample_tasks = {
            "Математика": [
                ...
                },
                {
                    ...
                    "description": "Найдите максимум f(x) = x³ - 3x² + 2 на [0; 3]",
                    "answer": "2",
            ...
            "Русский язык": [
                {
                    ...
                    "answer": "1",
                    "solution": 'В слове "старинный" пишется НН, так как образовано от существительного "старина"',
                    "difficulty": 1,
                },
                {
                    "title": "Обособленное определение",
            ...
            "Физика": [
                ...
                },
                {
                    ...
                    "description": "Найдите сопротивление проводника длиной 100 м, сечением 2 мм², ρ = 0,017 Ом·мм²/м",
                    "answer": "0,85 Ом",
        ...
        for subject_name, tasks in sample_tasks.items():
            try:
                subjects = Subject.objects.filter(name=subject_name)  # type: ignore
