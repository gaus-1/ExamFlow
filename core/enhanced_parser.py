...
class FipiParser:
    ...
    def get_tasks_for_subject(self, subject_url: str, subject_name: str) -> List[Dict]:
        ...
        try:
            ...
            task_elements = soup.find_all(['tr', 'li', 'div'], class_=lambda x: x and 'task' in str(x).lower())  # type: ignore
...
class ReshuEGEParser:
    ...
    def get_tasks_for_subject(self, subject_url: str, subject_name: str) -> List[Dict]:
        ...
        try:
            ...
            # Ищем задания
            task_elements = soup.find_all(['div', 'span'], class_=lambda x: x and 'task' in str(x).lower())  # type: ignore
...
class DataIntegrator:
    ...
    def create_or_update_subject(self, subject_data: Dict) -> Subject:
        ...
        try:
            ...
            exam_type = 'ЕГЭ' if any(word in subject_data['name'].lower() 
                                   for word in ['егэ', 'егэ', 'профиль', 'база']) else 'ОГЭ'
