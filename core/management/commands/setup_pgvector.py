        ...
        try:
            with connection.cursor() as cursor:
                ...
                if not cursor.fetchone():
                    self.stdout.write(
                        self.style.WARNING('Устанавливаем расширение pgvector...')  # type: ignore
                    )
                    cursor.execute("CREATE EXTENSION IF NOT EXISTS vector;")
                    self.stdout.write(
                        self.style.SUCCESS('✅ Расширение pgvector установлено')  # type: ignore
                    )
                else:
                    self.stdout.write(
                        self.style.SUCCESS('✅ Расширение pgvector уже установлено')  # type: ignore
                    )
                ...
                if not cursor.fetchone():
                    self.stdout.write(
                        self.style.ERROR('❌ Таблица core_datachunk не найдена. Сначала выполните миграции.')  # type: ignore
                    )
                    return
                ...
                if not cursor.fetchone():
                    self.stdout.write(
                        self.style.ERROR('❌ Поле embedding_vector не найдено. Выполните миграцию 0003_setup_pgvector.')  # type: ignore
                    )
                    return
                ...
                if options['force']:
                    self.stdout.write(
                        self.style.WARNING('Удаляем существующие индексы...')  # type: ignore
                    )
                ...
                self.stdout.write(
                    self.style.WARNING('Создаем ivfflat индекс для векторного поиска...')  # type: ignore
                )
                cursor.execute("""
                ...
                self.stdout.write(
                    self.style.WARNING('Создаем индексы для фильтрации...')  # type: ignore
                )
                cursor.execute("""
                ...
                cursor.execute("SELECT COUNT(*) FROM core_datachunk WHERE embedding_vector IS NOT NULL;")
                vector_count = cursor.fetchone()[0]

                self.stdout.write(
                    self.style.SUCCESS(  # type: ignore
                        f'📊 Статистика: {chunk_count} чанков, {vector_count} с векторами'
                    )
                )

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при настройке pgvector: {e}')  # type: ignore
            )
            return
