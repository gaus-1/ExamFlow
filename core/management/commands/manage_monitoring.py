...
class Command(BaseCommand):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при запуске мониторинга: {e}') # type: ignore
            )
    ...
    def stop_monitoring(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при остановке мониторинга: {e}') # type: ignore
            )
    ...
    def show_status(self):
        ...
        try:
            ...
            self.stdout.write(f'Сервис мониторинга: {status_icon} {"Запущен" if service.is_running else "Остановлен"}') # type: ignore      
            ...
            if service.is_running:
                ...
                if alerts_summary['recent_alerts']:
                    ...
                    for alert in alerts_summary['recent_alerts'][-5:]:
                        ...
                        self.stdout.write(f'  {level_icon} [{timestamp}] {alert["title"]}')
    ...
    def show_health(self):
        ...
        try:
            ...
            self.stdout.write(f'Общее состояние: {status_icon} {overall_status.upper()}')
            ...
            for check_name, check_result in health['checks'].items():
                ...
                self.stdout.write(f'  {status_icon} {check_name}: {check_result.get("message", "Нет информации")}')
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при получении состояния здоровья: {e}') # type: ignore
            )
    ...
    def show_alerts(self):
        ...
        try:
            ...
            for alert in active_alerts:
                ...
                self.stdout.write(f'{level_icon} [{alert.level.value.upper()}] {alert.title}')
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при получении уведомлений: {e}') # type: ignore
            )
    ...
    def test_alert(self, options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Неверный уровень уведомления: {level_str}') # type: ignore
            )
            return
        
        self.stdout.write(
            self.style.SUCCESS(f'🧪 Создание тестового уведомления уровня {level.value}...') # type: ignore
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ Тестовое уведомление создано: {alert.id}') # type: ignore
            )
            
            # Показываем детали
            self.stdout.write(f'   Уровень: {alert.level.value}')
            self.stdout.write(f'   Заголовок: {alert.title}')
            self.stdout.write(f'   Время: {alert.timestamp.strftime("%Y-%m-%d %H:%M:%S")}')
            
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при создании тестового уведомления: {e}') # type: ignore   
            )
