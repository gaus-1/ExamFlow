...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'🗄️ База данных: каждые {db_interval} секунд ({db_interval//60} минут)')
        self.stdout.write(f'🌐 Сайт: каждые {site_interval} секунд ({site_interval//60} минут)')
        self.stdout.write('=' * 50)
        ...
        if daemon:
            ...
            self.stdout.write(f'   kill {site_process.pid}  # Остановить keep-alive сайта')
            ...
            try:
                while True:
                    ...
                    self.stdout.write(f'⏰ Следующая проверка через {wait_time} секунд...')
