...
class Command(BaseCommand):
    ...
    def start_keepalive(self, options):
        ...
        if keepalive_service.is_running:
            self.stdout.write(self.style.WARNING(
                "Keepalive сервис уже запущен"))  # type: ignore
            return
        ...
        if options['daemon']:
            ...
            self.stdout.write(self.style.SUCCESS(
                "✅ Keepalive сервис запущен в фоне"))  # type: ignore
        else:
            self.stdout.write(
    ...
    def stop_keepalive(self):
        ...
        if not keepalive_service.is_running:
            self.stdout.write(self.style.WARNING(
                "Keepalive сервис не запущен"))  # type: ignore
            return
    ...
    def show_status(self):
        ...
        self.stdout.write("   Запущен: {'✅' if stats['is_running'] else '❌'}")

        if stats['stats']['last_check']:
            self.stdout.write("   Последняя проверка: {stats['stats']['last_check']}")

        if stats['stats']['last_success']:
            self.stdout.write("   Последний успех: {stats['stats']['last_success']}")

        self.stdout.write("   Всего проверок: {stats['stats']['checks_performed']}")
        self.stdout.write("   Успешных: {stats['stats']['successful_checks']}")
        self.stdout.write("   Неудачных: {stats['stats']['failed_checks']}")
        self.stdout.write(
            "   Последовательных неудач: {stats['stats']['consecutive_failures']}")

        if stats['stats']['checks_performed'] > 0:
            success_rate = (stats['stats']['successful_checks'] /
                            stats['stats']['checks_performed']) * 100
            self.stdout.write("   Процент успеха: {success_rate:.1f}%")
    ...
    def perform_check(self):
        ...
        self.stdout.write("📊 Общий статус: {health_results['overall_status']}")
        ...
        for component, result in health_results.items():
            ...
            if isinstance(result, dict):
                ...
                self.stdout.write("   {component}: {status_icon} {status}")

                if 'response_time' in result:
                    self.stdout.write(
                        "      Время ответа: {result['response_time']:.2f}s")

                if 'error' in result:
                    self.stdout.write("      Ошибка: {result['error']}")
    ...
    def wake_up_services(self):
        ...
        if website_ok and database_ok:
            self.stdout.write(self.style.SUCCESS(
                "✅ Все сервисы разбужены"))  # type: ignore
        else:
            self.stdout.write(self.style.WARNING(
                "⚠️ Некоторые сервисы не удалось разбудить"))  # type: ignore
