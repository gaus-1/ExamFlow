...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            documents = self._get_documents_to_process(
                data_type, force, limit)  # type: ignore
            ...
            self.stdout.write(
                # type: ignore
                self.style.ERROR('❌ Ошибка при обработке документов: {e}')  # type: ignore
            )
            logger.error('Ошибка обработки PDF документов: {e}')
    ...
    def _get_documents_to_process(self, data_type: str, force: bool, limit: int):
        ...
        query = query.filter(url__icontains='.pd')  # type: ignore
    ...
    def _show_documents_preview(self, documents):
        ...
        self.stdout.write('📋 Найдено {len(documents)} документов для обработки:')
        self.stdout.write('')

        for doc in documents:
            status = "✅ Обработан" if doc.is_processed else "⏳ Ожидает"  # type: ignore
            self.stdout.write('  • {doc.title}')
            self.stdout.write('    Тип: {doc.get_data_type_display()}')  # type: ignore
            self.stdout.write('    Статус: {status}')
            self.stdout.write('    URL: {doc.url}')
            self.stdout.write(
                '    Дата сбора: {doc.collected_at.strftime("%Y-%m-%d %H:%M")}')
            self.stdout.write('')
    ...
    def _process_documents(self, documents):
        ...
        self.stdout.write('🚀 Начинаем обработку {total_documents} документов...')
        self.stdout.write('')

        for i, document in enumerate(documents, 1):
            self.stdout.write(
                '[{i}/{total_documents}] Обрабатываем: {document.title[:50]}...'
            )
            ...
            try:
                ...
                if result['status'] == 'completed':
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(  # type: ignore
                            '  ✅ Успешно обработан: {result["chunks_saved"]} чанков сохранено'
                        )
                    )
                else:
                    failed_count += 1
                ...
                failed_count += 1
                self.stdout.write(
                    self.style.ERROR('  ❌ Исключение: {e}')  # type: ignore
                )
                logger.error('Ошибка обработки документа {document.id}: {e}')
        ...
        self.stdout.write('  Всего документов: {total_documents}')
        self.stdout.write('  Успешно обработано: {processed_count}')
        self.stdout.write('  Ошибок: {failed_count}')
