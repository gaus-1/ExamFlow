...
class KeepaliveService:
    ...
    def __init__(self):
        self.website_url = getattr(settings, 'WEBSITE_URL', 'https://examflow.onrender.com')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('ok'):
                    ...
                    logger.warning(f"⚠️ Telegram бот не отвечает: {data.get('description')}")
                    return False
            else:
                logger.warning(f"⚠️ Telegram API отвечает с кодом {response.status_code}")
                return False
        except Exception as e:
    ...
    def wake_up_database(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                result = cursor.fetchone()
                logger.info(f"✅ База данных разбужена, профилей: {result[0] if result else 0}")
                return True
        except Exception as e:
    ...
    def run_keepalive_cycle(self, interval=300):  # 5 минут
        ...
        while True:
            try:
                ...
                logger.info(f"{status} Статус компонентов: Сайт={website_ok}, БД={database_ok}, Бот={bot_ok}")
    ...
    def handle(self, *args, **options):
        ...
        if options['once']:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING("Keepalive сервис остановлен")) # type: ignore
