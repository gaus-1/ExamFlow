...
class Command(BaseCommand):
    ...
    def start_service(self):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔄 Запуск системы мониторинга изменений...') # type: ignore
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('✅ Система мониторинга изменений запущена') # type: ignore
            )
    ...
    def stop_service(self):
        ...
        self.stdout.write(
            self.style.WARNING('🛑 Остановка системы мониторинга изменений...') # type: ignore
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('✅ Система мониторинга изменений остановлена') # type: ignore
            )
        except Exception as e:
            self.stdout.write(
    ...
    def show_status(self):
        ...
        try:
            ...
            self.stdout.write(f'Сервис: {status_icon} {"Запущен" if service.is_running else "Остановлен"}') # type: ignore
    ...
    def check_changes(self, options):
        ...
        try:
            ...
            for change in changes:
                ...
                self.stdout.write(f'   Время: {change.timestamp.strftime("%Y-%m-%d %H:%M:%S")}')
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при проверке изменений: {e}') # type: ignore
            )
    
    def show_statistics(self):
        """Показывает подробную статистику"""
        self.stdout.write(
            self.style.SUCCESS('📈 Подробная статистика системы мониторинга') # type: ignore
        )
        self.stdout.write('=' * 60)
        ...
        try:
            ...
            self.stdout.write(f'  Статус: {"🟢 Запущен" if stats["is_running"] else "🔴 Остановлен"}')
    ...
    def show_recent_changes(self, options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'⏰ Недавние изменения за последние {hours} часов') # type: ignore
        )
        self.stdout.write('=' * 60)
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка при получении недавних изменений: {e}') # type: ignore
            )
