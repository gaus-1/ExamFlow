...
class Command(BaseCommand):
    ...
    def start_bot(self, daemon=False):
        ...
        if self.is_bot_running():
            self.stdout.write(self.style.WARNING("⚠️  Бот уже запущен!"))  # type: ignore
            return
    ...
    def stop_bot(self):
        ...
        if not os.path.exists(pid_file):
            self.stdout.write(
                self.style.WARNING(
                    "⚠️  PID файл не найден. Бот может быть не запущен."
                )  # type: ignore
            )
            return
    ...
    def show_status(self):
        ...
        if self.is_bot_running():
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "⚠️  Не удалось проверить API: {str(e)}"
                    )  # type: ignore
                )
        else:
            self.stdout.write(
