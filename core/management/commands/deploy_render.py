    ...
    def handle(self, *args, **options):
        ...
        if not self.check_database():
            self.stdout.write(self.style.ERROR(
                '❌ Не удалось подключиться к базе данных'))  # type: ignore
            return

        # Проверяем таблицы
        if not self.check_tables():
            self.stdout.write(self.style.ERROR(
                '❌ Проблемы с таблицами'))  # type: ignore
            return
        ...
        self.stdout.write(self.style.SUCCESS(
            '🎉 ДЕПЛОЙ ЗАВЕРШЕН УСПЕШНО!'))  # type: ignore
    ...
    def check_database(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                self.stdout.write(self.style.SUCCESS(
                    '✅ Подключение к базе данных успешно'))  # type: ignore
                return True
