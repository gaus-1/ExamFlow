...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not self.check_database():
            self.stdout.write(self.style.ERROR('❌ Не удалось подключиться к базе данных'))  # type: ignore
            return
        
        # Проверяем таблицы
        if not self.check_tables():
            self.stdout.write(self.style.ERROR('❌ Проблемы с таблицами'))  # type: ignore
            return
        ...
        self.stdout.write('=' * 50)
        self.stdout.write(self.style.SUCCESS('🎉 ДЕПЛОЙ ЗАВЕРШЕН УСПЕШНО!'))  # type: ignore
    ...
    def check_database(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                self.stdout.write(self.style.SUCCESS('✅ Подключение к базе данных успешно'))  # type: ignore
                return True
        except OperationalError as e:
            self.stdout.write(self.style.ERROR(f'❌ Ошибка подключения к базе данных: {e}'))  # type: ignore
            return False
    ...
    def check_tables(self):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'✅ Таблица Subject: {subject_count} записей'))  # type: ignore
            self.stdout.write(self.style.SUCCESS(f'✅ Таблица Task: {task_count} записей'))  # type: ignore
            
            return True
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'❌ Ошибка при проверке таблиц: {e}'))  # type: ignore
            return False
    ...
    def create_sample_data(self):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'❌ Ошибка при создании образцов данных: {e}'))  # type: ignore
            return False
