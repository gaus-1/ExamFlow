...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if success:
                ...
                self.stdout.write(f'📅 Завершено: {end_time.strftime("%Y-%m-%d %H:%M:%S")}')
    ...
    def _update_tasks_only(self):
        ...
        try:
            ...
            if not subjects:
                self.stdout.write('❌ Нет предметов в базе. Сначала запустите --subjects-only')
                return False
    ...
    def _show_statistics(self):
        ...
        try:
            ...
            for subject in Subject.objects.all(): # type: ignore
                subject_tasks = Task.objects.filter(subject=subject).count() # type: ignore
                subject_topics = Topic.objects.filter(subject=subject).count() # type: ignore
