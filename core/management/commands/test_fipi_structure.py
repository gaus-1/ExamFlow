...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not any([options['init_map'], options['test_scraping'], options['show_stats']]):
            self.show_help()
    ...
    def test_scraping(self, priority=None):
        ...
        if priority:
            ...
            self.stdout.write('Тестируем критически важные и высокоприоритетные источники')
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Тестирование завершено: {success_count} успешно, {error_count} ошибок')
        )
    ...
    def show_help(self):
        ...
        self.stdout.write('  --priority N   - Тестировать только источники с приоритетом N')
        self.stdout.write('')
        self.stdout.write('Примеры:')
        self.stdout.write('  python manage.py test_fipi_structure --init-map')
        self.stdout.write('  python manage.py test_fipi_structure --test-scraping --priority 1')
