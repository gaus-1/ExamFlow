...
from learning.models import Subject, Topic, Task, ExamType
...
class DataIntegrator:
    ...
    def update_all_data(self):
        ...
        try:
            with transaction.atomic():
                ...
                # 4. Обновляем решения
                solutions = self._update_solutions(subjects)
                
                logger.info(f"Обновлено: {len(subjects)} предметов, {len(topics)} тем, {len(tasks)} заданий")
                return True
        ...
        for subject in subjects:
            ...
            for topic_name in base_topics:
                topic, created = Topic.objects.get_or_create( # type: ignore
                    name=topic_name,
                    subject=subject,
                    defaults={'code': f"{subject.exam_type}_{subject.name[:3]}_{topic_name[:3]}"}
                )
                topics.append(topic)
                if created:
    ...
    def _create_tasks(self, subjects, topics):
        ...
        for subject in subjects:
            ...
            subject_tasks = self.fipi_parser.parse_tasks_for_subject(subject.name, subject.exam_type)
            
            for task_data in subject_tasks:
                # Находим подходящую тему
                topic = next((t for t in topics if t.subject == subject and task_data.get('topic') in t.name), None)
    ...
    def _update_solutions(self, subjects):
        ...
        for subject in subjects:
            solution_data = self.reshu_parser.get_solutions(subject.name, subject.exam_type)
