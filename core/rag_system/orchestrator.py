...
class RAGOrchestrator:
    ...
    def process_query(
        ...
        try:
            ...
            if cached_result:
                logger.info("Ответ получен из кеша для запроса: {query[:50]}...")
            ...
            if not search_results:
                ...
                if search_results:
                    self._cache_search_results(
                        query, subject, document_type, search_results)
                    logger.info(
                        "Результаты поиска сохранены в кеш для запроса: {query[:50]}...")
            else:
                logger.info(
                    "Результаты поиска получены из кеша для запроса: {query[:50]}...")
            ...
            logger.info("Запрос обработан за {result['processing_time']:.2f}с")
            return result
    ...
    def _generate_cache_key(
            ...
            subject: str,
            document_type: str,
            cache_type: str = "query") -> str:
        """
        Генерирует ключ кеша для запроса
        """
        content = "{query}|{subject}|{document_type}"
        hash_key = hashlib.md5(content.encode()).hexdigest()
        return "rag_{cache_type}:{hash_key}"

    def _get_cached_search_results(self,
                                   query: str,
                                   subject: str,
                                   document_type: str) -> Optional[List[Dict]]:
        """
    ...
    def _aggregate_context(self, search_results: List[Dict]) -> str:
        ...
        for result in search_results:
            ...
            context_parts.append("[{result['source_title']}] {result['text']}")
    ...
    def _generate_answer(self, query: str, context: str, subject: str) -> str:
        ...
        try:
            ...
            return "Не удалось сгенерировать ответ: {str(e)}"
    ...
    def _create_prompt(self, query: str, context: str, subject: str) -> str:
        ...
        """
        subject_context = " по предмету {subject}" if subject else ""

        return """Ты - эксперт по подготовке к ЕГЭ{subject_context}.
...
Ответ:"""
    ...
    def _update_user_profile(
        ...
        try:
            ...
            logger.error("Ошибка при обновлении профиля пользователя: {e}")
