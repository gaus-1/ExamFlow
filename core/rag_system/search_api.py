...
class FipiSearchAPIView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            rag = RAGOrchestrator()
            results = rag.process_query(prompt=query, subject=subject, limit=limit)
            ...
            return JsonResponse(
                {"error": "Внутренняя ошибка сервера", "results": [], "total": 0},
                status=500,
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not query:
                return JsonResponse(
                    {"error": "Поле query обязательно", "results": [], "total": 0},
                    status=400,
                )
            ...
            for source in results.get("sources", []):
                ...
                if (
                    "subject" in filters
                    and filters["subject"].lower()
                    not in source.get("subject", "").lower()
                ):
                    continue
            ...
            return JsonResponse(
                {"error": "Неверный JSON в теле запроса", "results": [], "total": 0},
                status=400,
            )
        except Exception as e:
...
def fipi_semantic_search(request):
    ...
    try:
        ...
        if not query:
            return JsonResponse(
                {"error": "Параметр q обязателен", "results": []}, status=400
            )
        ...
        rag = RAGOrchestrator()
        results = rag.process_query(prompt=query, subject=subject, limit=10)
        ...
        return JsonResponse({"error": "Ошибка поиска", "results": []}, status=500)
...
def search_suggestions(request):
    ...
    try:
        ...
        return JsonResponse({"suggestions": [], "query": query or ""})
