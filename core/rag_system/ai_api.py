...
class AIQueryView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not prompt:
                return JsonResponse(
                    {"error": "Поле prompt обязательно", "answer": "", "sources": []},
                    status=400,
                )
            ...
            if use_context:
                ...
                rag_results = rag.process_query(
                    prompt=prompt, subject=subject, user_id=user_id, limit=5
                )
...
"""
            ...
            return JsonResponse(
                {"error": "Неверный JSON в теле запроса", "answer": "", "sources": []},
                status=400,
            )
        except Exception as e:
            logger.error(f"Ошибка в AIQueryView: {e}")
            return JsonResponse(
                {"error": "Ошибка обработки AI запроса", "answer": "", "sources": []},
                status=500,
            )
...
class AIConversationView(View):
    ...
    def _build_conversation_context(
        self, message: str, history: list[dict[str, str]], subject: str
    ) -> str:
    ...
    def _save_conversation_turn(
        self, conversation_id: str, user_id: int, message: str, response: str
    ):
...
def ai_health_check(request):
    ...
    try:
        ...
        test_response = ai_orchestrator.ask(
            prompt="Тест", user_id=None, subject="", use_context=False
        )
        ...
        return JsonResponse(
            {"status": "unhealthy", "ai_available": False, "error": str(e)}, status=500
        )
...
def ai_capabilities(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Ошибка получения информации о возможностях"}, status=500
        )
...
def ai_subjects(request):
    ...
    try:
        ...
        return JsonResponse({"subjects": subjects, "total": len(subjects)})

    except Exception as e:
        logger.error(f"Ошибка в ai_subjects: {e}")
        return JsonResponse({"error": "Ошибка получения предметов"}, status=500)
...
def ai_user_profile(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Ошибка получения профиля"}, status=500)
...
def ai_problem_submit(request):
    ...
    try:
        ...
        if not problem_text:
            return JsonResponse(
                {"error": "Поле problem обязательно", "solution": "", "steps": []},
                status=400,
            )
...
"""
        ...
        ai_response = ai_orchestrator.ask(
            prompt=prompt, user_id=user_id, subject=subject, use_context=True
        )
        ...
        return JsonResponse(
            {"error": "Неверный JSON в теле запроса", "solution": "", "steps": []},
            status=400,
        )
    except Exception as e:
        logger.error(f"Ошибка в ai_problem_submit: {e}")
        return JsonResponse(
            {"error": "Ошибка решения задачи", "solution": "", "steps": []}, status=500
        )
...
def ai_statistics(request):
    ...
    try:
        stats = {
            "total_queries": 0,
            "queries_by_subject": {"математика": 0, "русский язык": 0},
            "queries_by_exam_type": {"ЕГЭ": 0, "ОГЭ": 0},
            "average_response_time": 0.0,
            "success_rate": 100.0,
        }

        return JsonResponse({"statistics": stats, "period": "all_time"})
