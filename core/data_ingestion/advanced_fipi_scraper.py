            ...
            try:
                ...
                if links:
                    ...
                    for link in links:
                        ...
                        detected_type = self.detect_content_type(link_url, link_title, page_content)
                        ...
                        # пытаемся найти внутри страницы реальные PDF-ссылки и сохранить их
                        if detected_type in ['demo_variant', 'specification', 'codefier'] and not link_url.lower().endswith('.pdf'):
                            try:
                                ...
                                for a in soup.find_all('a', href=True):
                                    ...
                                    if href.lower().endswith('.pdf'):
                                        ...
                                        pdf_links.append((href, a.get_text(strip=True) or link_title))

                                # Резервный поиск PDF через regex, если ничего не нашли
                                if not pdf_links:
                                    try:
                                        for href in re.findall(r"href=['\"]([^'\"]+\.pdf)['\"]", link_content, flags=re.IGNORECASE):
                                            norm = href
                                            if norm.startswith('/'):
                                ...
                                for pdf_url, pdf_title in pdf_links:
                                    ...
                                    if FIPIData.objects.filter(content_hash=pdf_hash).exists():  # type: ignore
                                        continue
                                ...
                                logger.warning(f"Не удалось извлечь PDF-ссылки с {link_url}: {e}")
                                # продолжаем обычную обработку как HTML

                        else:
                            # Обычная запись: HTML или прямая PDF
                            content_hash = self.get_content_hash((link_content or '')[:5000] + link_url)
                            if FIPIData.objects.filter(content_hash=content_hash).exists():  # type: ignore
                                continue
                            FIPIData.objects.create(  # type: ignore
                    ...
                    logger.info(f"С индекс-страницы {source.url} сохранено {saved_on_page} элементов")
                ...
                if not FIPIData.objects.filter(content_hash=content_hash).exists():  # type: ignore
                    FIPIData.objects.create(  # type: ignore
                        title=source.name,
                        url=source.url,
