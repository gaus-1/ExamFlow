...
class TaskQueue:
    ...
    def add_task(self, task: IngestionTask):
        ...
        with self._lock:
            ...
            logger.info(f"Задача {task.id} добавлена в очередь с приоритетом {task.priority.name}")
    ...
    def _execute_task(self, task: IngestionTask):
        ...
        try:
            ...
            if source.content_hash == content_hash:
                ...
                task.result_data = {'status': 'no_changes', 'content_hash': content_hash}
            else:
            ...
            if task.retry_count < task.max_retries:
                ...
                logger.info(f"Задача {task.id} будет повторена (попытка {task.retry_count}/{task.max_retries})")
            else:
                task.status = TaskStatus.FAILED
                logger.error(f"Задача {task.id} провалена после {task.max_retries} попыток")
...
class IngestionEngine:
    ...
    def add_source_tasks(self, source_ids: Optional[List[str]] = None, priority_filter: Optional[TaskPriority] = None):
    ...
    def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"Статистика движка: {json.dumps(stats, indent=2, default=str)}")
