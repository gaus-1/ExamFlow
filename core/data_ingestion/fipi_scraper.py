...
class FIPIScraper:
    ...
    def extract_demo_variants(self) -> List[Dict]:
        ...
        for link in links:
            ...
            if any(keyword in text.lower() for keyword in ['демо', 'демонстрационный', 'вариант']):
    ...
    def collect_all_data(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            logger.info(f"- Демонстрационных вариантов: {len(all_data['demo_variants'])}")
            logger.info(f"- Заданий из открытого банка: {len(all_data['open_bank_tasks'])}")
