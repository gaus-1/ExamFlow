...
from core.admin_views import parsing_status, start_parsing, trigger_parsing
...
from core.api import search_tasks as api_search_tasks
from core.sitemaps import RootViewSitemap, StaticViewSitemap
...
urlpatterns = [
    ...
    path(
        "api/subjects/<int:subject_id>/tasks/",
        get_tasks_by_subject,
        name="api_tasks_by_subject",
    ),
    ...
    path(
        "test-themes/",
        lambda request: render(request, "test_themes.html"),
        name="test_themes",
    ),
    # Демонстрация стилей
    path(
        "style-showcase/",
        lambda request: render(request, "style-showcase.html"),
        name="style_showcase",
    ),
    path(
        "aesop-showcase/",
        lambda request: render(request, "aesop-showcase.html"),
        name="aesop_showcase",
    ),
    # Новые страницы
    path(
        "features/", lambda request: render(request, "features.html"), name="features"
    ),
    path("pricing/", lambda request: render(request, "pricing.html"), name="pricing"),
    path(
        "subscribe/",
        lambda request: render(request, "subscribe.html"),
        name="subscribe",
    ),
    # Sitemap.xml
    path(
