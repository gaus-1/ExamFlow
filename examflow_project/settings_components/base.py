...
_ALLOWED_HOSTS_RAW = os.getenv(
    "ALLOWED_HOSTS",
    "examflow.ru,www.examflow.ru,localhost,127.0.0.1,testserver,examflow.onrender.com",
)
...
if USE_SQLITE_FALLBACK and RENDER_HOST:
    ...
    if "postgres" in _DATABASE_URL and (
        "render.com" in _DATABASE_URL
        or "35.227.164.209" in _DATABASE_URL
        or "dpg-d2dn09ali9vc73b2lg7g-a" in _DATABASE_URL
        or "dpg-" in _DATABASE_URL
    ):
        ...
        DATABASES["default"][
            "CONN_HEALTH_CHECKS"
        ] = True  # Проверка здоровья соединений
    elif "postgres" in _DATABASE_URL and (
        "localhost" in _DATABASE_URL or "127.0.0.1" in _DATABASE_URL
    ):
...
STATIC_VERSION = os.getenv("STATIC_VERSION", "v20250923-1")
