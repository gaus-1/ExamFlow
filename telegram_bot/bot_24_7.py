...
class ExamFlowBot24_7:
    ...
    async def setup_handlers(self):
        ...
        self.application.add_handler(
            CallbackQueryHandler(subjects_menu, pattern=r"^subjects$")
        )
        self.application.add_handler(
            CallbackQueryHandler(show_subject_topics, pattern=r"^subject_\d+$")
        )
        self.application.add_handler(
            CallbackQueryHandler(random_task, pattern=r"^random_task$")
        )
        self.application.add_handler(
            CallbackQueryHandler(show_answer, pattern=r"^show_answer_\d+$")
        )
        self.application.add_handler(
            CallbackQueryHandler(show_stats, pattern=r"^stats$")
        )
        self.application.add_handler(
            CallbackQueryHandler(learning_plan_menu, pattern=r"^learning_plan$")
        )

        # AI обработчики
        self.application.add_handler(
            CallbackQueryHandler(ai_help_handler, pattern=r"^ai_help$")
        )
        self.application.add_handler(
            CallbackQueryHandler(ai_explain_handler, pattern=r"^ai_explain_\d+$")
        )
        self.application.add_handler(
            CallbackQueryHandler(ai_personal_handler, pattern=r"^ai_personal$")
        )
        self.application.add_handler(
            CallbackQueryHandler(ai_hint_general_handler, pattern=r"^ai_hint_general$")
        )

        # Геймификация
        self.application.add_handler(
            CallbackQueryHandler(gamification_menu_handler, pattern=r"^gamification$")
        )
        self.application.add_handler(
            CallbackQueryHandler(user_stats_handler, pattern=r"^user_stats$")
        )
        self.application.add_handler(
            CallbackQueryHandler(achievements_handler, pattern=r"^achievements$")
        )
        self.application.add_handler(
            CallbackQueryHandler(progress_handler, pattern=r"^progress$")
        )
        self.application.add_handler(
            CallbackQueryHandler(
                overall_progress_handler, pattern=r"^overall_progress$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                subjects_progress_handler, pattern=r"^subjects_progress$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(
                daily_challenges_handler, pattern=r"^daily_challenges$"
            )
        )
        self.application.add_handler(
        ...
        self.application.add_handler(
            CallbackQueryHandler(bonus_handler, pattern=r"^bonus$")
        )

        # Поиск и навигация
        self.application.add_handler(
            CallbackQueryHandler(search_subject_handler, pattern=r"^search_subject_")
        )
        self.application.add_handler(
            CallbackQueryHandler(
                random_subject_handler, pattern=r"^random_subject_\d+$"
            )
        )
        self.application.add_handler(
            CallbackQueryHandler(show_task_handler, pattern=r"^task_\d+$")
        )
        self.application.add_handler(
            CallbackQueryHandler(clear_context_handler, pattern=r"^clear_context$")
        )

        # Авторизация
        self.application.add_handler(
            CallbackQueryHandler(telegram_auth_handler, pattern=r"^telegram_auth$")
        )
        self.application.add_handler(
            CallbackQueryHandler(auth_success_handler, pattern=r"^auth_success$")
        )
    ...
    async def restart_bot(self):
        ...
        if self.restart_count >= self.max_restarts:
            logger.error(
                f"❌ Превышено максимальное количество перезапусков ({self.max_restarts})"
            )
            return
