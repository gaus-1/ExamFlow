...
from core.personalization_system import PersonalizedRecommendations, get_user_insights
        ...
        keyboard = [
            [
                ...
                InlineKeyboardButton(
                    "🎯 Рекомендации", callback_data="my_recommendations"
                ),
            ],
            [
...
async def show_my_analytics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.edit_message_text(
            "❌ Произошла ошибка при получении аналитики.", parse_mode="Markdown"
        )
...
async def show_my_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i, task in enumerate(recommended_tasks, 1):
            ...
            subject_name = (
                getattr(task.subject, "name", "Неизвестно")
        ...
        await update.callback_query.edit_message_text(
            "❌ Произошла ошибка при получении рекомендаций.", parse_mode="Markdown"
        )
...
async def show_study_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.edit_message_text(
            "❌ Произошла ошибка при получении плана обучения.", parse_mode="Markdown"
        )
...
async def show_weak_topics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.callback_query.edit_message_text(
            "❌ Произошла ошибка при получении слабых тем.", parse_mode="Markdown"
        )
