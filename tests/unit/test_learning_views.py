...
class TestLearningViews(TestCase):
    ...
    def setUp(self):
        ...
        if not self.subject:
            self.subject = Subject.objects.create(  # type: ignore
                name="Тестовая Математика", description="Тестовая математика для тестов"
            )
    ...
    def test_submit_answer_view_correct_answer(self):
        ...
        request = self.factory.post(
            "/submit-answer/",
            data={"task_id": self.task.id, "answer": "1"},
    ...
    def test_submit_answer_view_incorrect_answer(self):
        ...
        request = self.factory.post(
            "/submit-answer/",
            data={"task_id": self.task.id, "answer": "2"},
            content_type="application/json",
        )
        request.user = self.user
    ...
    def test_submit_answer_view_missing_task_id(self):
        ...
        request = self.factory.post(
            "/submit-answer/", data={"answer": "1"}, content_type="application/json"
        )
        request.user = self.user
    ...
    def test_submit_answer_view_invalid_json(self):
        ...
        request = self.factory.post(
            "/submit-answer/", data="invalid json", content_type="application/json"
        )
        request.user = self.user
    ...
    def test_submit_answer_view_creates_progress(self):
        ...
        initial_count = UserProgress.objects.filter(
        ...
        final_count = UserProgress.objects.filter(
            user=self.user, task=self.task
        ).count()
