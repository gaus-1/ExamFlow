...
class TestCoreModels(TestCase):
    ...
    def setUp(self):
        ...
        self.user = TelegramUser.objects.create(
            telegram_id=123456789, telegram_username="testuser"
        )

        # Создаем профиль пользователя
        self.profile = UserProfile.objects.create(user=self.user)  # type: ignore
    ...
    def test_subject_creation(self):
        ...
        subject = Subject.objects.create(  # type: ignore
            name="Тестовый Русский язык", description="Тестовый русский язык для тестов"
        )
    ...
    def test_user_progress_str_method(self):
        ...
        progress = UserProgress.objects.create(  # type: ignore
            user=self.user, task=self.task, is_correct=True
        )
    ...
    def test_daily_challenge_str_method(self):
        ...
        challenge = DailyChallenge.objects.create(  # type: ignore
            title="Тестовый вызов", description="Описание вызова", challenge_type="math"
        )
    ...
    def test_user_challenge_creation(self):
        ...
        user_challenge = UserChallenge.objects.create(  # type: ignore
            user=self.user, challenge=challenge, current_progress=50
        )
    ...
    def test_user_challenge_progress_percentage(self):
        ...
        user_challenge = UserChallenge.objects.create(  # type: ignore
            user=self.user, challenge=challenge, current_progress=75
        )
    ...
    def test_fipi_source_map_str_method(self):
        ...
        source_map = FIPISourceMap.objects.create(  # type: ignore
            source_name="Тестовый источник", source_url="https://example.com"
        )
    ...
    def test_subject_choices(self):
        ...
        subject = Subject.objects.create(  # type: ignore
            name="Тестовый предмет", description="Описание", exam_type="oge"
        )
    ...
    def test_daily_challenge_choices(self):
        ...
        challenge = DailyChallenge.objects.create(  # type: ignore
            title="Тестовый вызов", description="Описание", challenge_type="russian"
        )
