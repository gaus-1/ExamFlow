...
class FrontendStandardsTestCase(TestCase):
    ...
    def test_css_minification_available(self):
        ...
        min_css_path = os.path.join(
            settings.STATIC_ROOT or "static", "css", "examflow-2.0.min.css"
        )
    ...
    def test_js_minification_available(self):
        ...
        min_js_path = os.path.join(
            settings.STATIC_ROOT or "static", "js", "core-functions.min.js"
        )
    ...
    def test_lazy_loading_images(self):
        ...
        self.assertContains(
            response,
            'loading="lazy"',
            msg_prefix="Lazy loading не найден в изображениях",
        )
    ...
    def test_css_variables_updated(self):
        ...
        css_path = os.path.join(
            settings.STATIC_ROOT or "static", "css", "examflow-2.0.css"
        )
        ...
        self.assertIn(
            "/* Размеры шрифтов (стандарт 2025: 3 основных размера) */", css_content
        )
    ...
    def test_flat_buttons_design(self):
        ...
        self.assertNotIn(
            "var(--gradient-primary)", css_content, "Градиенты не убраны из кнопок"
        )
...
class PerformanceTestCase(TestCase):
    ...
    def test_css_file_size(self):
        ...
        self.assertLess(
            min_css_size, css_size, "Минифицированный CSS не меньше обычного"
        )

        # Сжатие должно быть значительным (хотя бы 20%)
        compression_ratio = (css_size - min_css_size) / css_size
        self.assertGreater(
            compression_ratio, 0.2, "Сжатие CSS менее 20%: {compression_ratio:.1%}"
        )
    ...
    def test_js_file_size(self):
        ...
        js_path = os.path.join(
            settings.STATIC_ROOT or "static", "js", "core-functions.js"
        )
