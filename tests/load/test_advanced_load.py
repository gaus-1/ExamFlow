...
class ExamFlowUser(HttpUser):
    ...
    def ai_query(self):
        ...
        with self.client.post("/api/ai/ask/", 
                            json=data,
    ...
    def search_tasks(self):
        ...
        with self.client.get(f"/api/fipi/search/?q={query}&subject={self.current_subject}",
                           catch_response=True) as response:
            if response.status_code == 200:
                try:
                    result = response.json()
...
class ExamFlowAPIUser(HttpUser):
    ...
    def batch_search(self):
        ...
        for query in queries:
            with self.client.get(f"/api/fipi/search/?q={query}",
                               headers={"Authorization": f"Bearer {self.api_key}"},
                               catch_response=True) as response:
                if response.status_code == 200:
                    response.success()
    ...
    def concurrent_requests(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            if success_rate >= 0.8:  # 80% успешных запросов
                ...
                logger.warning(f"Low success rate in concurrent requests: {success_rate}")
...
class TestAdvancedLoadTesting:
    ...
    def test_stress_test(self):
        ...
        assert success_rate >= 85.0, f"Stress test success rate too low: {success_rate}%"
    ...
    def test_endurance_test(self):
        ...
        assert max_response_time < 30000, f"Max response time too high: {max_response_time}ms"
    ...
    def test_memory_leak_detection(self):
        ...
        for cycle in range(5):
            ...
            logger.info(f"Memory increase after cycle {cycle + 1}: {memory_increase:.2f}MB")
        ...
        assert total_memory_increase < 50, f"Significant memory leak: {total_memory_increase:.2f}MB"
    ...
    def test_concurrent_users_scalability(self):
        ...
        assert min_success_rate >= 85.0, f"Scalability degraded: min success rate {min_success_rate}%"
        
        # Время ответа не должно расти экспоненциально
        response_times = [r['avg_response_time'] for r in results]
        max_response_time = max(response_times)
        assert max_response_time < 10000, f"Response time degraded: max {max_response_time}ms"
