...
class ExamFlowUser(HttpUser):
    ...
    def register_user(self):
        ...
        try:
            ...
            response = self.client.post(
                "/telegram_auth/login/", data=auth_data, name="telegram_auth_login"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Ошибка регистрации пользователя {self.user_id}: {response.status_code}"
                )
    ...
    def view_subject_tasks(self, subject_id):
        ...
        response = self.client.get(
            f"/learning/subjects/{subject_id}/tasks/", name="subject_tasks"
        )
    ...
    def view_task_detail(self, task_id):
        ...
        response = self.client.get(f"/learning/tasks/{task_id}/", name="task_detail")
    ...
    def check_user_progress(self):
        ...
        response = self.client.get(
            f"/api/user/{self.telegram_id}/progress/", name="user_progress"
        )
        ...
        if response.status_code == 200:
            try:
                ...
                if subjects:
                    ...
                    self.client.get(
                        f'/api/subjects/{subject["id"]}/tasks/',
