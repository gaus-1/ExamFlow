...
def run_load_tests():
    ...
    for config in test_configs:
        ...
        cmd = [
            ...
            "--host",
            "http://localhost:8000",  # Измените на ваш URL
            "--users",
            str(config["users"]),
            "--spawn-rate",
            ...
            "--headless",
            "--html",
            f'reports/load_test_{config["class"]}.html',
            "--csv",
            f'reports/load_test_{config["class"]}',
            "--class",
            config["class"],
        ]
...
def run_single_test(users=10, duration="2m", test_class="WebsiteUser"):
    ...
    cmd = [
        ...
        "--headless",
        "--html",
        f"reports/load_test_{test_class}.html",
        "--csv",
        f"reports/load_test_{test_class}",
        "--class",
        test_class,
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--users", type=int, default=10, help="Количество пользователей"
    )
