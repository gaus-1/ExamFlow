...
class TestTelegramWebhook:
    ...
    def test_webhook_invalid_json(self, client):
        ...
        response = client.post(
            "/bot/webhook/", data="invalid json", content_type="application/json"
        )
...
class TestTelegramBotHandlers:
    ...
    def test_help_handler(self, mock_get_bot, user):
        ...
        assert (
            "помощь" in call_args[1]["text"].lower()
            or "help" in call_args[1]["text"].lower()
        )
...
class TestTelegramBotGamification:
    ...
    def test_points_award(self, mock_gamification, user):
        ...
        mock_instance.award_points.assert_called_once_with(
            user.telegram_id, "task_completed", 10
        )
    ...
    def test_achievement_unlock(self, mock_gamification, user):
        ...
        mock_instance = Mock()
        mock_gamification.return_value = mock_instance
        mock_instance.check_achievements.return_value = [
        ...
        mock_instance.check_achievements.assert_called_once_with(
            user.telegram_id, "task_completed"
        )
    ...
    def test_level_up(self, mock_gamification, user):
        ...
        mock_instance = Mock()
        mock_gamification.return_value = mock_instance
        mock_instance.get_user_level.return_value = {
            "level": 2,
            "points": 150,
...
class TestTelegramBotAI:
    ...
    def test_ai_error_handling(self, mock_container, user):
        ...
        assert response is not None
        assert (
            "ошибка" in response["answer"].lower()
...
class TestTelegramBotCommands:
    ...
    def test_command_parsing(self):
        ...
        assert parse_command("/ai Помогите с математикой") == (
            "ai",
...
class TestTelegramBotNotifications:
    ...
    def test_send_achievement_notification(self, mock_get_bot, user):
        ...
        from telegram_bot.bot_handlers import (
            send_achievement_notification,  # type: ignore
        )
...
class TestTelegramBotErrorHandling:
    ...
    def test_webhook_error_handling(self, client):
        ...
        invalid_data = {"invalid": "data", "structure": "test"}
    ...
    def test_ai_service_error_handling(self, mock_container, user):
        ...
        assert (
            "ошибка" in response["answer"].lower()
            or "недоступен" in response["answer"].lower()
        )
...
class TestTelegramBotIntegration:
    ...
    def test_full_user_journey(self, client, user):
        ...
        response = client.post(
            "/bot/webhook/", data=json.dumps(ai_data), content_type="application/json"
        )
        assert response.status_code == 200
