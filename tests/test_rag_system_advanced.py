...
class TestRAGOrchestratorCaching(TestCase):
    ...
    def test_cache_key_generation(self):
        ...
        key1 = self.orchestrator._generate_cache_key(query, subject, document_type, "query")
        key2 = self.orchestrator._generate_cache_key(query, subject, document_type, "search")
        key3 = self.orchestrator._generate_cache_key(query, subject, document_type, "ai_response")
        ...
        key1_dup = self.orchestrator._generate_cache_key(query, subject, document_type, "query")
    ...
    def test_search_results_caching(self):
        ...
        mock_results = [
            {'text': 'тест текст 1', 'source': 'источник 1'},
            {'text': 'тест текст 2', 'source': 'источник 2'}
        ]
        
        # Сохраняем в кэш
        self.orchestrator._cache_search_results(query, subject, document_type, mock_results)
        
        # Получаем из кэша
        cached_results = self.orchestrator._get_cached_search_results(query, subject, document_type)
    ...
    def test_ai_response_caching(self):
        ...
        # Сохраняем в кэш
        self.orchestrator._cache_ai_response(query, subject, document_type, mock_response)
        
        # Получаем из кэша
        cached_response = self.orchestrator._get_cached_ai_response(query, subject, document_type)
    ...
    def test_process_query_with_caching(self, mock_vector_store):
        ...
        mock_vector_store.return_value.semantic_search.return_value = [
            {'text': 'тест текст', 'source': 'источник'}
        ]
        
        # Мокаем генерацию ответа
        with patch.object(self.orchestrator, '_generate_answer', return_value='тест ответ'):
            with patch.object(self.orchestrator, '_format_sources', return_value=['источник']):
                ...
                self.assertEqual(mock_vector_store.return_value.semantic_search.call_count, 1)
