...
class TestBrowserCompatibility:
                ...
                if form.is_displayed():
                    ...
                    for input_field in inputs:
                        if input_field.is_displayed():
                            ...
                            if input_field.get_attribute("type") in [
                                "text",
                                "email",
                                "textarea",
                            ]:
                                try:
                                    ...
                                    logger.warning(
                                        f"Input test failed in {browser_name}: {e}"
                                    )
    ...
    def test_performance_comparison_across_browsers(self, browser_driver, live_server):
        ...
        logger.info(
            f"{browser_name} JavaScript performance: {js_time:.2f}s for 100 operations"
        )
    ...
    def test_mobile_viewport_in_all_browsers(self, browser_driver, live_server):
        ...
        if viewport_meta:
            ...
            assert (
                "width=device-width" in viewport_content
                or "initial-scale=1" in viewport_content
            )
