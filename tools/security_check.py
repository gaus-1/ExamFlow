...
def print_status(message, status="INFO"):
    ...
    if status == "SUCCESS":
        ...
        print("{Colors.YELLOW}⚠️  {message}{Colors.ENDC}")
    elif status == "INFO":
...
def check_ssl_certificate(url):
    ...
    try:
        ...
        if response.status_code == 200:
            print_status("SSL сертификат валиден для {url}", "SUCCESS")
            return True
        else:
            print_status("SSL работает, но статус {response.status_code}", "WARNING")
            return True
    ...
    try:
        ...
        for header, description in security_headers.items():
            if header in headers:
                print_status("  {description}: {headers[header]}", "SUCCESS")
            else:
        ...
        if settings_file.exists():
            ...
            for setting in security_settings:
                if setting in content:
                    print_status("  {setting}: настроен", "SUCCESS")
                else:
        ...
        for file_path in bot_files:
            ...
            if file.exists():
                ...
                for check in security_checks:
                    if check in content:
                        print_status("  {file_path}: {check} найден", "SUCCESS")
                    else:
                        print_status("  {file_path}: {check} не найден", "WARNING")
            else:
