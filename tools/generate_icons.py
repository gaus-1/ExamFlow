...
def create_simple_icon(size):
    ...
    book_height = int(
        book_width * 0.6
    )  # Corrected: book_height should be proportional to book_width
    book_x = margin
    book_y = margin
    ...
    for i in range(5):
        ...
        draw.rectangle(
            [line_x, line_y, line_x + line_width, line_y + 2], fill=(255, 254, 242, 255)
        )
        line_y += line_height
    ...
    draw.ellipse(
        [
            brain_x - brain_radius,
            brain_y - brain_radius,
            brain_x + brain_radius,
            brain_y + brain_radius,
        ],
        fill=(248, 248, 248, 255),
        outline=(37, 37, 37, 255),
        width=2,
    )
    ...
    draw.ellipse(
        [
            brain_x - brain_radius // 2 - eye_radius,
            brain_y - eye_radius,
            brain_x - brain_radius // 2 + eye_radius,
            brain_y + eye_radius,
        ],
...
def main():
    ...
    for size in sizes:
        print("📱 Создание иконки {size}x{size}...")

        # Создаем иконку
        icon = create_simple_icon(size)

        # Сохраняем PNG
        filename = "{icons_dir}/icon-{size}x{size}.png"
