...
def main():
    ...
    print(f"🎯 Django SECRET_KEY ({len(django_key)} символов):")
    print(f"SECRET_KEY={django_key}")
    print()
    
    # Генерируем кастомный SECRET_KEY
    custom_key = generate_custom_secret_key(64)
    print(f"🔑 Кастомный SECRET_KEY ({len(custom_key)} символов):")
    print(f"SECRET_KEY={custom_key}")
    print()
    
    # Проверяем безопасность
    print("✅ Проверка безопасности:")
    print(f"   Длина Django ключа: {len(django_key)} символов")
    print(f"   Длина кастомного ключа: {len(custom_key)} символов")
    print(f"   Минимальная длина: 50 символов")
