...
class ThemeUsage(models.Model):
    ...
    page_views = models.PositiveIntegerField(
        default=0, verbose_name="Количество просмотренных страниц"
    )
...
class ThemeCustomization(models.Model):
    ...
    custom_colors = models.JSONField(
        default=dict, blank=True, verbose_name="Пользовательские цвета"
    )

    custom_fonts = models.JSONField(
        default=dict, blank=True, verbose_name="Пользовательские шрифты"
    )

    is_active = models.BooleanField(
        default=False, verbose_name="Пользовательские настройки активны"
    )
