...
def demo_theme_management():
    ...
    if created:
        print(f"✅ Созданы пользовательские настройки для темы: {customization.get_theme_display()}")
    else:
        print(f"📝 Обновлены пользовательские настройки для темы: {customization.get_theme_display()}")
    ...
    if customization.is_active:
        ...
        print(f"   Основной цвет: {customization.get_custom_color('primary', '#000000')}")
