...
class UserThemePreferenceAdmin(admin.ModelAdmin):
    ...
    list_display = ["user", "theme", "is_active", "created_at", "updated_at"]

    list_filter = ["theme", "is_active", "created_at", "updated_at"]
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("user", "theme", "is_active")}),
        (
...
class ThemeUsageAdmin(admin.ModelAdmin):
    ...
    list_filter = ["theme", "created_at", "session_duration", "page_views"]
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("user", "theme")}),
        ("Статистика", {"fields": ("session_duration", "page_views")}),
...
class ThemeCustomizationAdmin(admin.ModelAdmin):
    ...
    def deactivate_customizations(self, request, queryset):
        ...
        updated = queryset.update(is_active=False)
        self.message_user(
            request, f"Деактивировано {updated} пользовательских настроек"
        )
